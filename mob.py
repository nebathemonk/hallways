...
#/////////////Combat procs//////////////////////
  #using a skill  in combat
    def on_attack(self,skill,target):
        ...
        while i < skillhits:
            ...
            totaldamage = target.on_defend(damage)
 ...
 #called when a mob is attacked in combat
    def on_defend(self,damage):
        dfs = self.get_defense()
        #todo: code looking for damage type, resistance types, etc, for full defense code later on
        print self.name+" defends with "+str(dfs)+" defense. ["+str(int(dfs/2))+"]"
        totaldamage = int(damage - (dfs/2))
...
#leveling up and class change procs
    def on_levelup(self):
        ...
        self.health = int((100*self.growth_health)*self.level)
        self.stamina = int((100*self.growth_stamina)*self.level)
        self.attack = int((50*self.growth_attack)*self.level/2)
        self.defense = int((50*self.growth_defense)*self.level/2)
        self.accuracy = int((50*self.growth_accuracy)*self.level/2)
        self.evasion = int((50*self.growth_evasion)*self.level/2)
